...
static block_t *BlockAMT(stream_t *p_access, bool *restrict eof)
{
    ...
    int i_tunnel = UDP_HDR_LEN + AMT_HDR_LEN + (sys->mcastGroupAddr.ipv4.sin_family == AF_INET ? IP_HDR_LEN : IPv6_FIXED_HDR_LEN );
    ...
    /* Otherwise pull native multicast */
        ...
        len = recvfrom( sys->fd, (char *)pkt->p_buffer, sys->mtu + i_tunnel, 0, (struct sockaddr*)&temp, &temp_size );
        if ( len <= 0 )
        {
...
static bool open_amt_tunnel( stream_t *p_access )
{
    ...
    for ( server = serverinfo; server != NULL && !vlc_killed(); server = server->ai_next )
    {
        ...
        if( unlikely( inet_ntop(server->ai_family, binary_address, psz_relay_ip,INET6_ADDRSTRLEN) == NULL ) )
        {
            int i_errConv = errno;
        ...
        msg_Dbg( p_access, "Trying AMT Server: %s", psz_relay_ip);
